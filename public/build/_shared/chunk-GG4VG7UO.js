...
var require_use_sync_external_store_shim_development = __commonJS({
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim.development.js"(exports) {
    ...
    (function() {
      ...
      var React3 = require_react(), objectIs = "function" === typeof Object.is ? Object.is : is, useState5 = React3.useState, useEffect = React3.useEffect, useLayoutEffect = React3.useLayoutEffect, useDebugValue = React3.useDebugValue, didWarnOld18Alpha = false, didWarnUncachedGetSnapshot = false, shim = "undefined" === typeof window || "undefined" === typeof window.document || "undefined" === typeof window.document.createElement ? useSyncExternalStore$1 : useSyncExternalStore$2;
...
// src/app/stores/StoreContext.tsx
...
function storeAnnotation(prototype3, key, annotation) {
  if (!hasProp(prototype3, storedAnnotationsSymbol)) {
    addHiddenProp(prototype3, storedAnnotationsSymbol, _extends({}, prototype3[storedAnnotationsSymbol]));
  }
  if (isOverride(annotation) && !hasProp(prototype3[storedAnnotationsSymbol], key)) {
    var fieldName = prototype3.constructor.name + ".prototype." + key.toString();
    die("'" + fieldName + "' is decorated with 'override', but no such decorated member was found on prototype.");
  }
  assertNotDecorated(prototype3, annotation, key);
  if (!isOverride(annotation)) {
    prototype3[storedAnnotationsSymbol][key] = annotation;
  }
}
function assertNotDecorated(prototype3, annotation, key) {
  if (!isOverride(annotation) && hasProp(prototype3[storedAnnotationsSymbol], key)) {
    var fieldName = prototype3.constructor.name + ".prototype." + key.toString();
    var currentAnnotationType = prototype3[storedAnnotationsSymbol][key].annotationType_;
...
function makeIterableForMap(iterator2) {
  iterator2[Symbol.toStringTag] = "MapIterator";
  return makeIterable(iterator2);
}
...
function makeIterableForSet(iterator2) {
  iterator2[Symbol.toStringTag] = "SetIterator";
  return makeIterable(iterator2);
}
...
function makeIterable(iterator2) {
  ...
  return Object.assign(Object.create(maybeIteratorPrototype), iterator2);
}
function getSelf() {
  return this;
}
...
isFetchSupported && ((res) => {
  ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type) => {
    !resolvers[type] && (resolvers[type] = utils_default.isFunction(res[type]) ? (res2) => res2[type]() : (_14, config) => {
...
var simulateLogin = async (username, password) => {
  const users = [
  ...
  if (user) {
    const response = { success: true, message: "Login successful", token: "fake-jwt-token-123" };
    return response;
  } else {
    const response = { success: false, message: "Invalid username or password" };
    return response;
  }
};
var fetchRickAndMortyCharactersAPI = async () => {
  const response = await axios_default.get("https://rickandmortyapi.com/api/character");
...
// src/app/stores/CharacterStore.ts
  ...
  get filteredCharacters() {
    ...
    if (this.searchTerm) {
      filtered = filtered.filter(
        (character) => character.name.toLowerCase().includes(this.searchTerm.toLowerCase())
      );
    }
    if (this.selectedStatus !== "all") {
      filtered = filtered.filter((character) => character.status === this.selectedStatus);
    }
    if (this.selectedSpecies !== "all") {
      filtered = filtered.filter((character) => character.species === this.selectedSpecies);
    }
    return filtered;
  }
  get uniqueStatuses() {
...
var UIStore = class {
  ...
  addNotification(message, type = "info", duration = 5e3) {
...
var StoreProvider = ({ children }) => {
  const rootStore = (0, import_react.useMemo)(() => {
    const store = new RootStore();
    return store;
  }, []);
  return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(StoreContext.Provider, { value: rootStore, children }, void 0, false, {
    fileName: "src/app/stores/StoreContext.tsx",
    lineNumber: 14,
    columnNumber: 5
  }, this);
};
...
use-sync-external-store/cjs/use-sync-external-store-shim.development.js:
   ...
   *)
*/
//# sourceMappingURL=/build/_shared/chunk-GG4VG7UO.js.map
